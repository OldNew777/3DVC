...
def render(
    ...
    imageio.mimsave('images/render_cube.gif', [np.uint8(im * 255) for im in all_images])
...
def train(
    ...
    render_images(
        ...
        save=True, file_prefix='images/render_box_before_training'
    )
    ...
    render_images(
        model, cameras, image_size,
        save=True, file_prefix='images/render_box_after_training'
    )
    all_images = render_images(
        model, create_surround_cameras(3.0, n_poses=20), image_size, file_prefix='box'
    )
    imageio.mimsave('images/render_box.gif', [np.uint8(im * 255) for im in all_images])


def create_model(cfg, mode='train'):
    # Create model
    model = Model(cfg)
    model.cuda()
    if mode == 'eval':
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        print("Epoch: {}, Mean Loss: {}".format(epoch, mean_loss / cnt))
        ...
        if (
            ...
            with torch.no_grad():
                ...
                imageio.mimsave('images/render_nerf.gif', [np.uint8(im * 255) for im in test_images])
...
def render_nerf(
    ...
    # Create model
    model = create_model(cfg, mode='eval')

    # Render
    with torch.no_grad():
        test_images = render_images(
            model, create_surround_cameras(3.0, n_poses=20, up=(0.0, 0.0, 1.0), focal_length=2.0),
            cfg.data.image_size, file_prefix='nerf'
        )
    imageio.mimsave('images/render_nerf.gif', [np.uint8(im * 255) for im in test_images])
