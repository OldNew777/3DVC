...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    # TODO (1): Map pixels to points on the image plane at Z=1
    ndc_points = torch.stack((xy_grid[:, 0], xy_grid[:, 1]), dim=-1)
    ...
    rays_o = camera.get_camera_center() + torch.zeros_like(world_space_points[..., :1])
